...
scrape_wiki_pages (wiki_soup, state, state_abbr, hdate):
 ...
 def scrape_state (mainurl, state, state_abbr):
    ...
    df1.to_csv('USAF-COVID-19/WIKIPEDIA/' + state_abbr + '/DAILY/' + datetime.now().strftime('%Y-%m-%d') + '-COVID-19-WIKI-' + state_abbr + '.csv',index=False)
    ...
    for index, page in pagesdf.iterrows():
        ...
        fn = 'USAF-COVID-19/WIKIPEDIA/' + state_abbr + '/DAILY/' + page.Date.strftime('%Y-%m-%d') + '-COVID-19-WIKI-' + state_abbr + '.csv'
